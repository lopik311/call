...
Значит самый простой путь — запустить **отдельный Python TCP-сервер** на `127.0.0.1:9092` и читать аудио напрямую из AudioSocket.
...
Скрипт примет соединение и начнет выводить `[PARTIAL]` и `[FINAL]`.

> По умолчанию сервер работает в цикле и принимает звонки один за другим.
> Для одиночного запуска используйте `--once`.
...
## Если звонок сразу срывается

Запустите с `--verbose` и проверьте:
1. Есть ли строка `AudioSocket connected: ...`.
2. Есть ли `AudioSocket UUID frame: ...`.
3. Нет ли `[ERROR] AudioSocket remote error: ...`.
4. Нет ли предупреждения `[WARN] Аудио-фреймы не получены...` (часто это codec/формат).
